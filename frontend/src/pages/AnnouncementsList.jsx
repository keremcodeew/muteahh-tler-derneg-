...
const PLACEHOLDER = 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=600&q=80';
...
export default function AnnouncementsList() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
      <div className="mb-8">
        <h1 className="font-serif text-3xl md:text-4xl font-semibold text-dark-gray">Duyurular</h1>
        <p className="mt-2 text-gray-600">Dernek duyuruları ve etkinlikler</p>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-gray-500 py-12 text-center">Henüz duyuru yok.</p>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.items.map((item) => (
              <Link key={item.id} to={`/announcements/${item.id}`} className="card group overflow-hidden">
                ...
                <div className="p-5">
                  ...
                  {item.eventDate && (
                    <span className="text-xs text-primary ml-2">Etkinlik: {formatDate(item.eventDate)}</span>
                  )}
